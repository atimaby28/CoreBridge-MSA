...
## 1-1. 왜 Outbox Pattern인가?

서비스 간 이벤트 전파 방식을 선택할 때 4가지 대안을 비교 분석했다.

| 대안 | 동작 방식 | 문제점 | 판정 |
|------|----------|--------|------|
| **Dual Write** | 비즈니스 로직에서 DB 저장 + Kafka 발행을 순차 호출 | Kafka 발행 실패 시 데이터 유실, 2PC 없이는 원자성 보장 불가 | ❌ |
| **@TransactionalEventListener** | Spring 이벤트로 트랜잭션 커밋 후 Kafka 발행 | JVM 재시작/크래시 시 메모리 이벤트 유실, 재시도 메커니즘 없음 | ❌ |
| **CDC (Debezium)** | DB 변경 로그(WAL)를 캡처하여 Kafka 전달 | 13개 서비스에 Debezium Connector 각각 설정 → 인프라 복잡도 과도, 16GB RAM 환경에서 부담 | ❌ |
| **Outbox + Polling** | 비즈니스 로직과 동일 트랜잭션으로 Outbox 테이블 저장 → 별도 스레드가 Kafka 발행 | DB 트랜잭션으로 원자성 보장 + 실패 시 10초 내 재시도 | ✅ |

### Polling 주기 10초의 근거
- **5초 미만**: DB 폴링 부하 증가 (4개 서비스 × 4개 샤드 = 16회/5초)
- **30초 이상**: 장애 복구 지연이 사용자 체감 수준
- **10초**: `AFTER_COMMIT` 즉시 발행이 기본 경로이므로 폴링은 실패 복구용. 10초면 사용자가 인지하기 전 복구 가능

### 참조 구현과의 차이
kuke-board 원본의 Outbox 구조를 CoreBridge에 맞게 변환:
- **EventType**: 게시판 이벤트 → 채용공고/댓글/좋아요/조회 이벤트 (8종)
- **Topic**: 단일 토픽 → 5개 토픽 분리 (서비스 도메인별 + 알림)
- **Payload**: 공고의 requiredSkills/preferredSkills 필드 추가 (AI 매칭용)
...
> **구현 노트**: 설계 시 `data-serializer`, `event`, `outbox-message-relay`를 독립 모듈로 계획했으나,
> Gradle 멀티모듈 빌드 단순화를 위해 단일 `common` 모듈 내 패키지로 통합 구현했습니다.
> (`settings.gradle`에서 `include('common')`으로 관리)
...
| `consumer/JobpostingReadEventConsumer.java` | @KafkaListener, 다중 토픽 구독 |
...
|------|------|
| `config/KafkaConfig.java` | KafkaListener 설정 |
| `consumer/JobpostingHotEventConsumer.java` | @KafkaListener, 다중 토픽 구독 |
| `service/event/handler/EventHandler.java` | 핸들러 인터페이스 |
...
| `corebridge-notification` | apply | notification | NOTIFICATION_CREATED |
...
1. `common` 모듈 내 data-serializer 패키지 구현
2. `common` 모듈 내 event 패키지 구현 (EventType, Payload 클래스들)
3. `common` 모듈 내 outbox-message-relay 패키지 구현
